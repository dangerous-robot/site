"""Comparison logic for consistency checks: verdict ordering, severity, needs_review."""
...
def _verdict_severity(actual: Verdict, assessed: Verdict) -> VerdictSeverity:
    """Determine the severity of a verdict disagreement."""
    if actual == assessed:
        return VerdictSeverity.MATCH

    a_ordinal = VERDICT_ORDER.get(actual)
...
def _confidence_distance(actual: Confidence, assessed: Confidence) -> int:
    """Number of steps apart on the confidence scale."""
    return abs(CONFIDENCE_ORDER[actual] - CONFIDENCE_ORDER[assessed])
...
def compare(
    ...
    """Compare an LLM assessment against actual claim metadata."""
    verdict_agrees = actual_verdict == assessment.verdict
    confidence_agrees = actual_confidence == assessment.confidence
    severity = _verdict_severity(actual_verdict, assessment.verdict)
    conf_dist = _confidence_distance(actual_confidence, assessment.confidence)
