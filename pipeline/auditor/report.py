"""Report generation for auditor results."""
...
def format_text_report(results: list[ComparisonResult]) -> str:
    ...
    lines.append(f"{'Claim':<40} {'Primary':<14} {'Assessed':<14} {'Severity':<10} {'Review'}")
    lines.append("-" * 90)
    for r in results:
        review_flag = "***" if r.needs_review else ""
        lines.append(
            f"{r.claim_id:<40} {r.primary_verdict.value:<14} "
    ...
    if disagreements:
        ...
        for r in disagreements:
            ...
            lines.append(f"Primary:  verdict={r.primary_verdict.value}, confidence={r.primary_confidence.value}")
