...
| 2026-05-05 | claude-opus-4-7 (1M) | implementation | Plan fully implemented across five commits. Phase 1 badge demotion (commit 17422c9): `.kind-badge` and `.source-type-badge` switched to tinted chips with 15% color-mix tints, semantic-color borders + text; inline-style call sites switched to `--kind-color` custom property. Phase 2 PageHeader refactor (commit b889ff8): `variant` prop, outdent + rail for detail, inline-stack for index, container-query-driven breakpoint, `Astro.slots.has('default')` for rail gating, print fallback, new `--icon-box-size` / `--icon-box-size-row` tokens. **Plan deviation:** `container-type: inline-size` placed on `<body>` rather than `<main>` because `<main>`'s width is capped by `--content-max-width`, which would have prevented the min-width: 1000px query from ever matching. Light-theme icon-box uses `--color-bg` rather than `--color-surface` for visible inner contrast. Phase 3 index pages (commit f589d88): all 7 index pages adopted `<PageHeader variant="index">`. Phase 4 per-row icons (audit only, no commit): every list either has a colored badge (ClaimRow, SourceRow, CriteriaMatrix) or runs on a mono-typed page (EntityCard variants); rule applied as written, no icons added. Phase 5 (commit a0016ba): prototype page deleted. Browser-side visual QA across the verification matrix is still pending. |
