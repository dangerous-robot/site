...
**Status**: ✅ Complete. Parts 1, 2, and 3 implemented and gated (Part 1 verified 2026-04-26; Part 2 commit 54702c8, 2026-04-27; Part 3 implemented 2026-05-10; Part 3 live-run gate met per operator confirmation 2026-07-09).
...
Per-agent model selection landed via `--researcher-model`, `--analyst-model`, `--auditor-model`, `--ingestor-model` (and matching `DR_*_MODEL` env vars). `VerifyConfig` carries the four optional override fields plus `model_for(agent)` / `all_models()` helpers. The Mistral `reasoning_content` scrubber is implemented as a model-profile injection (`OpenAIModelProfile(openai_chat_send_back_thinking_parts=False)`) inside `resolve_model` for any spec whose model_id contains "mistral". `_check_provider_api_keys` now takes a list and validates the union across agent slots. Tests and CLI smoke pass; an end-to-end mixed-tier run was attempted and surfaced two gateway findings below.

### Out-of-band findings (post-implementation runs)

**gemma3n cannot do tool calling on Infomaniak.** The gateway returns HTTP 400 on `tool_choice: "auto"` ("requires `--enable-auto-tool-choice` and `--tool-call-parser` to be set"). Plain completion works. Treat gemma3n as structured-output-only (analyst/auditor) until Infomaniak enables tool calling. The tool-free researcher/ingestor variants tracked in `docs/plans/drafts/tool-free-researcher-ingestor_stub.md` would unblock gemma3n for all four agent roles by moving Brave search and HTTP fetch out of the model.

**gpt-oss-120b emits tool-call intent in `reasoning` instead of `tool_calls`.** Confirmed from `logs/debug.log` for a failing `dr verify-claim` run with `DR_RESEARCHER_MODEL=infomaniak:openai/gpt-oss-120b`: the assistant message comes back with `content: None`, populated `reasoning` field containing JSON-shaped tool-call descriptions (e.g. `{"query": "..."}`), and **no `tool_calls` field at all**. PydanticAI sees no tool call, retries up to the budget, and aborts with "Exceeded maximum retries (2) for output validation". The model sometimes self-corrects on retry but not reliably. This is a model+gateway behavior pattern, not a wiring bug.

Operational impact:
- `gpt-oss-120b` on Infomaniak: usable for **analyst and auditor** (structured-output via `final_result`-shaped tool calls works), unsuitable for **researcher and ingestor** (multi-turn tool loops fail).
- Recommended Part 2 mapping until either Infomaniak fixes the serialization or the tool-free variants land:
  - `DR_MODEL=anthropic:claude-haiku-4-5-20251001` (default, used by researcher + ingestor)
  - `DR_ANALYST_MODEL=infomaniak:openai/gpt-oss-120b`
  - `DR_AUDITOR_MODEL=infomaniak:mistral24b`
- The original Part 2 "Recommended target mapping" table below (researcher/ingestor on Infomaniak) is invalidated by these findings; revisit after the tool-free plan lands.

**Mistral model id update.** Infomaniak's current valid list does not include `mistralai/Mistral-Small-3.2-24B-Instruct-2506`. Use the short alias `infomaniak:mistral24b` instead. The scrubber still fires (it matches any model_id containing "mistral").
...
### Status: ✅ Complete (implemented 2026-05-10; live-run gate met per operator confirmation 2026-07-09)
...
| 2026-07-09 | human (Brandon) | Part 3 gate close | Confirmed the GreenPT live-run gate was exercised. Marked Part 3 (and the plan overall) complete; moved from `docs/plans/` to `docs/plans/completed/`. |
