...
**Existing partial mitigation:** `_run_with_null_retry` in `pipeline/orchestrator/pipeline.py:797` retries on this exact error pattern (2 retries, 45s delay). Used by the analyst and auditor paths; **not yet wired into the ingest path** (`_ingest_url` at `pipeline.py:655` calls `ingestor_agent.run(...)` directly).
