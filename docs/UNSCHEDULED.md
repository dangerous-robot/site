...
| Fix sector claim output path in `persistence.py` | Running `dr verify-claim sectors/ai-llm-producers "..."` writes files to `research/claims/aillm-producers/` instead of `research/claims/sectors/ai-llm-producers/`. Root cause: slug is derived from entity name ("AI/LLM Producers" → "aillm-producers") rather than the `entity_ref` path prefix. Fix: when `entity_ref` is available (e.g. `sectors/ai-llm-producers`), use its directory path as the claims output prefix instead of regenerating from the entity name. Discovered 2026-05-01; stray `research/claims/aillm-producers/` directory is an artifact from an affected run. |
